 ...
 */
const compressSync = 
  (text, { encoding = DEFAULT_ENCODING } = {} ) => zlib.deflateSync(text).toString(encoding);
...
const decompressSync = 
  (text, { encoding = DEFAULT_ENCODING } = {}) => zlib.inflateSync(Buffer.from(text, encoding)).toString();
