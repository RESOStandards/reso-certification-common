...
const createSecretKey = (
  secretText = process.env.CONFIG_FILE_SHARED_SECRET,
  { base2KeySize = 32 } = {}
) => {
...
const encryptSync = (
  ...
  return Buffer.from(
    JSON.stringify({
      iv: iv.toString('hex'),
      data: Buffer.concat([
        cipher.update(Buffer.from(useCompression ? compressSync(text) : text, textEncoding)),
        cipher.final()
      ]).toString('hex')
    }),
    textEncoding
...
const decryptSync = (
  ...
  const decoded = Buffer.concat([decipher.update(Buffer.from(data, 'hex')), decipher.final()]);

  return (
    useCompression ? decompressSync(Buffer.from(decoded, 'hex').toString(textEncoding)) : decoded
  ).toString();
};
